...
function getDayName(date) {
  ...
  return daysArray[newDate.getDay()];
}
...
function getNextFriday(date) {
  ...
  const tillFriday = startDay === 5 ? 7 : (5 - startDay + 7) % 7;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dayStart = new Date(dateStart).getTime() / 1000 / 60 / 60 / 24;
  const dayEnd = new Date(dateEnd).getTime() / 1000 / 60 / 60 / 24;
  if (dayStart > dayEnd) {
    return dayStart - dayEnd + 1;
  }
...
function formatDate(date) {
  ...
  const result = new Intl.DateTimeFormat('en-US', options).format(newDate);
  return result;
}
...
function getWeekNumberByDate(/* date */) {
  ...
  const miliseconds = (currentWeek - firstWeek) / 86400000;
  const result = Math.floor(miliseconds / 7) + 1;
  return result;
  */
}
